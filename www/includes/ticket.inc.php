...
if (isset($_POST['new-tickt-submit'])) {
  ...
  if (empty($telefone) || empty($assunto) || empty($msg)) {
    ...
    $sql = "SELECT t.idTicket,t.idUsuario,u.idUsers, u.nome,t.assunto from tickets as t inner join users as u on(t.idUsuario=u.idUsers) where t.assunto=?";
    ...
    if (!mysqli_stmt_prepare($stmt, $sql)) {
      ...
      if ($resultCheck > 0) {
        ...
        if (!mysqli_stmt_prepare($stmt, $sql)) {
          ...
          // 1 para novo
          $estado = 1;
          mysqli_stmt_bind_param($stmt, "isssss", $id_usuario, $email, $assunto, $msg, $telefone, $estado);
          mysqli_stmt_execute($stmt);
          header("Location: ../home.php?nonvoticket=success");
          exit();
        }
